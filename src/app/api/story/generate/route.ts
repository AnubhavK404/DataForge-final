...
function executeHonestAnalysis(
  ...
  // --- 1. Variable Locking (Mirroring Frontend Selection) ---
  ...
  if (Math.abs(bestR) > 0.4) {
    ...
    keyInsights.push(`${bestPair[0]} and ${bestPair[1]} are ${dir} (r ≈ ${bestR.toFixed(2)})${sampleSuffix}. This association is the strongest statistical link identified.`);
  }
  ...
  // --- 2. Chart–Text Consistency (Hard Variable Locking) ---
  const trend = (() => {
    if (targetCat && targetNum) {
      return {
        chartType: "bar" as const,
        text: `The average ${targetNum} per ${targetCat} shows how the primary metric is distributed across different categories.`
      };
    }
    return {
      chartType: "pie" as const,
      text: `Composition analysis shows how the volume of records is distributed across your primary categories.`
    };
  })();
  ...
  const pattern = (() => {
    if (Math.abs(bestR) > 0.5) {
      return {
        ...
        text: `The scatter plot of ${bestPair[0]} vs ${bestPair[1]} reveals their statistical association; no causality is established by this distribution.`
      };
    } else if (numericCols.length >= 3) {
      return {
        chartType: "correlation_heatmap" as const,
        text: `A cross-variable correlation matrix identifies which numeric factors are statistically associated and which move independently.`
      };
    }
    return {
      chartType: "bar" as const,
  ...
  // --- 3. Honest Recommendations (No Hallucinated Variables) ---
  ...
  return {
    ...
    keyInsightsChartType: pattern.chartType === "correlation_heatmap" ? "bar" : pattern.chartType,
    trendAnalysis: trend,
